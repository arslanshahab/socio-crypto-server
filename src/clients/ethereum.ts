...
export const listCoinGeckoTokens = async () => {
    ...
    for (let i = 0; i < resp.length; i++) {
        ...
        await RedisClient.getRedis().set(
            `TOKEN:ADDRESS:${resp[i].symbol.toLowerCase()}`,
            resp[i].platforms.ethereum != null ? resp[i].platforms.ethereum : ""
        );
    }
...
export const getEthPriceInUSD = async () => {
    const resp = await fetch(`${v3BaseUrl}/simple/price?ids=ethereum&vs_currencies=usd`, {
        method: "get",
