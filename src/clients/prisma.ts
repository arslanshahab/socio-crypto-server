...
const { PG_URL, PG_READER_URL, PG_HOST, PG_PORT, PG_USER, PG_PASSWORD, PG_HOST_READER } = process.env;
...
export const readPrisma = new PrismaClient({
    datasources: {
        db: {
            url: PG_READER_URL || `postgresql://${PG_USER}:${PG_PASSWORD}@${PG_HOST_READER}:${PG_PORT}/postgres`,
        },
    },
});
