...
export class withdrawValueOnTransfer1611777431808 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        await queryRunner.query(`UPDATE "transfer" t SET "usdAmount" = (SELECT CAST(o.amount as decimal) * 0.1 FROM public.transfer o WHERE t.id=o.id) WHERE action = 'withdraw' AND "status" IN ('PENDING','APPROVED') AND "currency" = 'coiin'`);
        await queryRunner.query(`UPDATE "transfer" t SET "usdAmount" = (SELECT o.amount FROM public.transfer o WHERE t.id=o.id) WHERE action = 'withdraw' AND "status" IN ('PENDING','APPROVED') AND "currency" = 'usd'`);
