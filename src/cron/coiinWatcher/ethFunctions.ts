...
const ethConnectionUrl =
    NODE_ENV === "production"
        ? "https://mainnet.infura.io/v3/c817284c8c504027a5701f2a6bc94c7f"
        : "https://ropsten.infura.io/v3/c817284c8c504027a5701f2a6bc94c7f";
...
export const checkForTokenTransactionsOnContract = async (
    ...
    const filteredContractTransactions = await (
        await fetch(ethConnectionUrl, getFilterPostParams(lastBlockChecked, currentBlock, contractAddress))
    ).json();
    logger.info(JSON.stringify(filteredContractTransactions));
    if (filteredContractTransactions.error)
        throw new Error(filteredContractTransactions.error.message || "error occurred getting response from ETH node");
    logger.info(`ETH RESPONSE: ${JSON.stringify(filteredContractTransactions)}`);
    if (filteredContractTransactions.result.length === 0)
        logger.info(`NO TRANSACTIONS FOUND IN ${currentBlock - lastBlockChecked} BLOCKS`);
    return filteredContractTransactions.result.length > 0
        ? filteredContractTransactions.result.map((txn: any) => {
              const convertedValue = txn.value
                  ? new BN(txn.value).div(decimalConversionValue)
                  : new BN(txn.data).div(decimalConversionValue);
              const newTransaction = new CryptoTransaction();
