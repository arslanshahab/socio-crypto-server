...
export class HourlyCampaignMetric extends BaseEntity {
    ...
    public static parseHourlyCampaignMetrics(
        hourlyMetrics: HourlyMetricsGroupedByDateQueryResult[],
        filter: DateTrunc,
        ...
        try {
            for (const metric of hourlyMetrics) {
                ...
                const averageDiscoveryCost = parseFloat(
                    discoveryCost
                        .div(Number(metric.likeCount) + Number(metric.shareCount) + Number(metric.commentCount))
                        .toString()
                );
                const averageConversionCost = parseFloat(
                    conversionCost
                        .div(Number(metric.submissionCount) + Number(metric.viewCount) + Number(metric.clickCount))
                        .toString()
                );
                ...
                response.push({
                    ...
                    totalDiscoveries:
                        Number(metric.likeCount) + Number(metric.shareCount) + Number(metric.commentCount),
                    totalConversions:
                        Number(metric.clickCount) + Number(metric.viewCount) + Number(metric.submissionCount),
    ...
    public static parseHourlyPlatformMetrics(
        hourlyMetrics: HourlyMetricsGroupedByDateQueryResult[],
        filter: DateTrunc
    ) {
    ...
    public static async upsert(
        ...
        switch (action) {
            ...
            case "comments":
                ...
                break;
            case "participate":
                record.participantCount = record.participantCount
                    ? record.participantCount.plus(new BN(actionCount))
                    : new BN(actionCount);
                break;
            case "post":
    ...
    public static async getDateGroupedMetrics(
        dateTrunc: DateTrunc,
        startDate: string,
        endDate: string,
        campaignId?: string
    ): Promise<HourlyMetricsGroupedByDateQueryResult[]> {
