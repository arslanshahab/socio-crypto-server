...
import {
    StringifiedArrayTransformer,
    BigNumberEntityTransformer,
...
export class Campaign extends BaseEntity {
    ...
    isOpen() {
        const now = new Date();
        if (
            new Date(this.beginDate).getTime() <= now.getTime() &&
    ...
    public static parseAlgorithm(algorithmEntity: AlgorithmSpecs) {
        ...
        for (const key in algorithm["pointValues"]) {
            algorithm["pointValues"][key] =
                algorithm["pointValues"][key].toString();
        }
        for (const tier in algorithm["tiers"]) {
            ...
            if (threshold !== "" && totalCoiins !== "") {
                algorithm["tiers"][tier]["threshold"] =
                    algorithm["tiers"][tier]["threshold"].toString();
                algorithm["tiers"][tier]["totalCoiins"] =
                    algorithm["tiers"][tier]["totalCoiins"].toString();
            }
        }
        return algorithm;
    }
    ...
    public asV1() {
        ...
        if (this.participants && this.participants.length > 0)
            returnedCampaign.participants = this.participants.map(
                (participant) => participant.asV1()
            );
        if (this.payouts && this.payouts.length > 0)
            returnedCampaign.payouts = this.payouts.map((payout) =>
                payout.asV1()
            );
        if (this.posts && this.posts.length > 0)
            returnedCampaign.posts = this.posts.map((post) => post.asV1());
    ...
    public static async getAllParticipatingCampaignIdsByUser(
        user: User
    ): Promise<string[]> {
        ...
        return u.campaigns.length > 0
            ? u.campaigns.map(
                  (participant: Participant) => participant.campaign.id
              )
            : [];
    }
    ...
    public static async findCampaignsByStatus(
        ...
        if (approved)
            query = query.andWhere('"status"=:status', { status: "APPROVED" });
        if (pendingAudit)
            query = query.andWhere('"audited"=:audited', { audited: false });
    ...
    public static async listCampaignsByStatus(
        open: boolean = true,
    ...
    public static async adminListCampaignsByStatus(
        skip: number,
        take: number,
    ...
    public static async getCampaignMetrics(id: string) {
        const { clickCount, submissionCount, viewCount, participants } =
            await this.createQueryBuilder("campaign")
                .leftJoinAndSelect(
                    "campaign.participants",
                    "participant",
                    'participant."campaignId" = campaign.id'
                ...
                .getRawOne();
        const { likes, shares, comments, posts } =
            await this.createQueryBuilder("campaign")
                .leftJoinAndSelect(
                    "campaign.posts",
                    "post",
    ...
    public static async getPlatformMetrics() {
        ...
        return {
            ...
            discoveryCount:
                Number(likes) + Number(shares) + Number(comments) || 0,
            conversionCount:
                Number(clickCount) +
    ...
    public static async updateAllDailyParticipationMetrics(campaignId: string) {
        ...
        for (let i = 0; i < campaign.participants.length; i++) {
            ...
            if (!alreadyHandledParticipants[participant.id]) {
                const metrics =
                    await DailyParticipantMetric.getSortedByParticipantId(
                        participant.id
                    );
                if (metrics.length > 0) {
                    if (
                        formatUTCDateForComparision(
                            metrics[metrics.length - 1].createdAt
