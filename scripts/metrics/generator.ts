...
export const main = async () => {
    ...
    for (let campaigns = 0; campaigns < 10; campaigns++) {
        ...
        for (let participants = 0; participants < totalParticipants; participants++) {
            ...
            const participant = await Participant.createNewParticipant(user, campaign);
            ...
            for (let hours = 0; hours < totalHours; hours++) {
                ...
                let hourlyClickCount = checkFrequency(24 / dailyMetrics.posts.frequency, hours + 1)
                    ? new BN(getRandomInt(dailyMetrics.clicks.max))
                    : new BN(0);
                let hourlyViewCount = checkFrequency(24 / dailyMetrics.views.frequency, hours + 1)
                    ? new BN(getRandomInt(dailyMetrics.views.max))
                    : new BN(0);
                let hourlySubmissionCount = checkFrequency(24 / dailyMetrics.submissions.frequency, hours + 1)
                    ? new BN(getRandomInt(dailyMetrics.submissions.max))
                    : new BN(0);
                const hourlyParticipantCount = new BN(getRandomInt(10));
                participantCount = participantCount.plus(hourlyParticipantCount).isLessThan(totalParticipants)
                    ? participantCount.plus(hourlyParticipantCount)
                    : new BN(totalParticipants);
                ...
                participantMetrics.participationScore = participantMetrics.participationScore
                    .plus(hourlyLikeCount)
                    .plus(hourlyShareCount)
                    .plus(hourlyCommentCount)
                    .plus(hourlyClickCount)
                    .plus(hourlyViewCount)
                    .plus(hourlySubmissionCount);
                ...
                if (hourlyMetricsArray.length < hours + 1) {
                    ...
                    hourlyMetricsArray[hours].submissionCount =
                        hourlyMetricsArray[hours].submissionCount.plus(hourlySubmissionCount);
                    ...
                    hourlyMetricsArray[hours].commentCount =
                        hourlyMetricsArray[hours].commentCount.plus(hourlyCommentCount);
