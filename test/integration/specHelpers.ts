...
export const createParticipant = async (runningApp: Application, options?: { [key: string]: any } | any) => {
    ...
    participant.user =
        getValue(["user"], options) || (await createUser(runningApp, getValue(["userOptions"], options)));
    participant.campaign =
        getValue(["campaign"], options) || (await createCampaign(runningApp, getValue(["campaignOptions"], options)));
...
export const createUser = async (runningApp: Application, options?: { [key: string]: any } | any) => {
    ...
    user.profile =
        getValue(["profile"], options) || (await createProfile(runningApp, getValue(["profileOptions"], options)));
    user.active = getValue(["active"], options) || true;
    user.posts = getValue(["posts"], options) || [];
    user.campaigns = getValue(["campaigns"], options) || [];
    user.wallet =
        getValue(["wallet"], options) || (await createWallet(runningApp, getValue(["walletOptions"], options)));
    ...
    user.notificationSettings =
        getValue(["notificationSettings"], options) ||
        (await createNotificationSettings(runningApp, getValue(["notificationOptions"], options)));
...
export const createExternalAddress = async (runningApp: Application, options?: { [key: string]: any } | any) => {
    ...
    externalWallet.ethereumAddress =
        getValue(["ethereumAddress"], options) || "0x0000000000000000000000000000000000000000";
...
const orderedListForAccountRecovery = (
...
export const getAccountRecoveryRequest = (keypair: IdentityKeyPair, recoveryCode: string, recoveryMessage: string) => {
    ...
    if (keypair.sign)
        structure.signature = keypair.sign(
            sha256Hash(Buffer.concat(orderedListForAccountRecovery(keypair, recoveryCode, recoveryMessage, timestamp)))
        );
    return structure;
};
...
export const getIdentityLoginRequest = (keypair: IdentityKeyPair) => {
    ...
    if (keypair.sign) {
        ...
        structure.signature = keypair.sign(
            sha256Hash(
                Buffer.concat(
                    orderedListForIdentityLogin(keypair, structure.identity.signature.signature, new Date(timestamp))
                )
            )
        );
    }
...
export const createIdentity = (): IdentityKeyPair => {
    ...
    keypair.sign = (hashedMessage: any) =>
        Buffer.from(
            secp256k1.ecdsaSign(Buffer.from(hashedMessage, "base64"), Buffer.from(keypair.privateKey, "base64"))
                .signature
        ).toString("base64");
    return keypair;
};
